...
Deno.test({
  ...
  await t.step("Action: getTopArtists - with valid parameters", async () => {
    ...
    assertEquals(
      typeof result.artists[0].artist_mbid,
      "string",
      "artist_mbid should be string or null",
    );
  });
  ...
  await t.step("Action: getTopReleases - with valid parameters", async () => {
    ...
    assertEquals(Array.isArray(result.releases[0].artist_mbids), true);
    assertEquals(Array.isArray(result.releases[0].artists), true);
    if (result.releases[0].artists.length > 0) {
      assertExists(result.releases[0].artists[0].artist_credit_name);
      assertExists(result.releases[0].artists[0].artist_mbid);
    }
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      assertEquals(Array.isArray(result.releaseGroups[0].artist_mbids), true);
      assertEquals(Array.isArray(result.releaseGroups[0].artists), true);
    },
  );
  ...
  await t.step("Action: getListenHistory - with minTimestamp", async () => {
    ...
    if (result.listens && result.listens.length > 0) {
      ...
      assertExists(result.listens[0].inserted_at);
      assertExists(result.listens[0].recording_msid);
      assertExists(result.listens[0].user_name);
    }
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      // Activity should be an array of listening activity periods
      assertEquals(Array.isArray(result.activity), true);
      // Each period should have the expected structure
      if (result.activity.length > 0) {
        assertExists(result.activity[0].from_ts);
        assertExists(result.activity[0].to_ts);
        assertExists(result.activity[0].time_range);
        assertEquals(typeof result.activity[0].listen_count, "number");
      }
    },
  );
