---
timestamp: 'Sat Oct 18 2025 11:02:33 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_110233.f7d5bfd3.md]]'
content_id: 3d7957c5a5eec662cc794576b4888a2a565da21e5aeee6155d67752bd22dfa19
---
...
```typescript
// file: src/ListenBrainzAPI/ListenBrainzAPIConcept.ts
...
export default class ListenBrainzAPIConcept {
  ...
  private async _getOrFetchTopItems<T>(
    ...
    // 3. Fetch from API
    const result = await this._callListenBrainzAPI<{ [key: string]: T[] }>(
      scrobbleToken,
      endpoint,
      { range: timeRange, limit: count },
    );
    ...
    const items = result[apiResponseKey] as T[] || [];
  ...
  async getListenHistory(
    ...
    const result = await this._callListenBrainzAPI<{ listens: Listen[] }>(
      scrobbleToken,
      `users/${username}/listens`,
      apiParams,
    );
    ...
    await this.listenHistoryCache.updateOne(
      { user },
      { $set: { listens: result.listens, lastFetched: new Date() }, $setOnInsert: { _id: freshID(), user } },
      { upsert: true },
    );
  ...
  async getListeningActivity(
    ...
    // 2. Fetch from API
    const result = await this._callListenBrainzAPI<{ activity: ListeningActivity }>(
      scrobbleToken,
      `users/${username}/listening-activity`,
      { range: timeRange },
    );
  ...
  async validateToken(
    ...
    try {
      ...
      return { valid: true, username: profile.user_name }; // Profile endpoint returns user_name
    } catch (e) {
