---
timestamp: 'Fri Oct 17 2025 00:56:30 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251017_005630.b161ab48.md]]'
content_id: 3e6e41a500739bbd96d6aed6f3744a04fc847925568b419cdf573615575227e0
---
...
Interfaces with the ListenBrainz API to fetch user listening data, including top artists, albums, tracks, and detailed listen history. Implements caching to reduce API calls.
...
Interfaces with MusicBrainz API to fetch comprehensive information about artists, recordings, releases, and works. Provides relationship data crucial for generating recommendations. Implements rate limiting (1 req/sec) and caching.
...
* `POST /api/auth/register` → User.createUser()
* `POST /api/auth/login` → User.startSession()
* `POST /api/auth/logout` → User.endSession()
* `POST /api/auth/link-listenbrainz` → User.associateToken() + validateToken()

**Statistics**:

* `GET /api/stats/artists?range=week&count=50` → ListenBrainzAPI.getTopArtists()
* `GET /api/stats/albums?range=month&count=50` → ListenBrainzAPI.getTopReleaseGroups()
* `GET /api/stats/tracks?range=all_time&count=100` → ListenBrainzAPI.getTopRecordings()
* `GET /api/stats/history?min_ts=...&count=50` → ListenBrainzAPI.getListenHistory()
* `GET /api/stats/activity?range=year` → ListenBrainzAPI.getListeningActivity()
* `GET /api/stats/dashboard?range=month` → getDashboard() sync

**Music Details**:

* `GET /api/music/artist/:mbid` → MusicBrainzAPI.lookupArtist()
* `GET /api/music/album/:mbid` → MusicBrainzAPI.lookupReleaseGroup()
* `GET /api/music/track/:mbid` → MusicBrainzAPI.lookupRecording()
* `GET /api/music/search?q=...&type=artist` → MusicBrainzAPI.searchEntities()

**Recommendations**:

* `POST /api/recommendations/generate` → generateSmartRecommendations() sync
  * Body: `{ sourceMbid: string, amount: number }`
* `GET /api/recommendations/:itemMbid` → Recommendation.getRecommendations()
* `POST /api/recommendations/feedback` → submitRecommendationFeedback() sync
  * Body: `{ recommendedMbid: string, isPositive: boolean }`
* `DELETE /api/recommendations/clear` → Recommendation.clearRecommendations()
